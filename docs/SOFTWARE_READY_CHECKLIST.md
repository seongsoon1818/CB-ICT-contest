...
`MODEL_SMOKE=PASS`의 범위는 checkpoint hash, bundle load, synthetic JPEG 추론, adapter 변환, readiness와 로컬 recording Backend의 HTTP 201 event handoff입니다. 실제 Spring Backend·운영 네트워크와 야생동물 정확도·현장 threshold는 `NOT_RUN` 또는 `NOT_VERIFIED`이며 Mock 결과도 실제 model 품질 증거로 사용하지 않습니다. 미완료 policy 항목과 이슈 상태는 별도로 판정합니다.
