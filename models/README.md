...
현재 구현이 지원하는 조합은 PyTorch checkpoint, `ultralytics-8.4.125`, `torch==2.13.0`, `torchvision==0.28.0`, RGB 640x640 letterbox 입력과 `ultralytics-yolo-detect-v1`입니다. 다른 runtime, output adapter, detector task, class ID 집합 또는 대소문자를 제외하고 classCode와 일치하지 않는 checkpoint class name은 시작 시 거절합니다. 새 플랫폼은 고정된 runtime 의존성을 설치한 뒤 실제 checkpoint smoke를 다시 통과해야 합니다. 모델 바이너리는 전달·배포 경로에만 두며 저장소 metadata 디렉터리는 그 자체로 완전한 실행 bundle이 아닙니다.
...
3. release 디렉터리에서 `sha256sum -c SHA256SUMS`로 checkpoint를 검증합니다.
